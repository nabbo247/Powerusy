...
var createPDFThumbnails = function () {
    ...
    if (!loaded && typeof (pdfjsLib) === 'undefined') {
        ...
        console.log("pdfjs location", src);
        if (!src) {
        ...
        nodesArray.forEach(function (element) {
            ...
            pdfjsLib.getDocument({ url: filePath, worker: worker }).promise.then(function (pdf) {
                pdf.getPage(1).then(function (page) {
                    ...
                    var error = "Could not open page 1 of the selected document. Confirm that the document is a PDF file.";
                    alert(error);
                    console.log("pdfThumbnails error: could not open page 1 of document " + filePath + ". Not a pdf ?");
                });
            }).catch(function () {
                var error = "Could not find or read the selected document. Confirm that the document is a PDF file.";
                alert(error);
